...
def save_plot(fig, filename):
    fig.savefig(os.path.join(PLOT_PATH, filename + '.jpg'),
    ...
    if plot_speed:
        ...
        for i in range(T_outbound-1):
            ...
            if i % 100 == 0:
                if i == 0:
                    continue
                plt.text(x[i],y[i],str(i),size=5)
                save_plot(fig, 'route/example_route_'+str(i))
...
def plot_traces(log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
    ...
    for i, cell_type in enumerate(include):
        ...
        if cell_type in ['CL1', 'CPU4', 'CPU1']:
            ax[i].set_ylim([0,16])
        elif cell_type in ['TB1']:
            ax[i].set_ylim([0,8])
        elif cell_type in ['TN2']:
            ax[i].set_ylim([0,1])
    ...
    for i, v_idx in enumerate(v_indices):
        ...
        coord1 = transFigure.transform(ax[0].transData.transform([v_idx,
                                                                  y_max]))
        coord2 = transFigure.transform(ax[4].transData.transform([v_idx, -3]))
        if i == 0 or i == 3:
            lw = 1
            zorder = 0
        else:
        ...
        line = ax[4].axvline(x=v_idx, lw=lw, c='#333333', linestyle='dashed')
        line.set_dashes(dashes)
