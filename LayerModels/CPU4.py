...
class CPU4(BaseLayerModel):
    updates = ['r']
    accesses = ['I'] #mem_update before clip
    params = ['slope','bias','cpu4_mem_gain']
    internals = OrderedDict([('V',0.5)])
    ...
    def pre_run(self, update_pointers):
        ...
        if self.params_dict.has_key('init_V'):
            cuda.memcpy_dtod(self.internal_states['V'].gpudata,
...
__global__ void update(
    ...
    %(slope)s* g_slope,
    %(bias)s* g_bias,
    %(cpu4_mem_gain)s* g_cpu4_mem_gain,
    %(V)s* g_V,
    %(r)s* g_r)
{
    ...
    for(int i = tid; i < num_comps; i += total_threads)
    {
        ...
        slope = g_slope[i];
        bias = g_bias[i];
        cpu4_mem_gain = g_cpu4_mem_gain[i];
