...
def update_cells(heading, velocity, tb1, memory, cx, filtered_steps=0.0):
    ...
    motion."""
    # Compass
    tl2 = cx.tl2_output(heading) #OK
    cl1 = cx.cl1_output(tl2) #OK
    tb1 = cx.tb1_output(cl1, tb1)#OK

    # Speed
    flow = cx.get_flow(heading, velocity, filtered_steps) 
    tn1 = cx.tn1_output(flow) #OK, not used
    tn2 = cx.tn2_output(flow)  #OK

    # Update memory for distance just travelled
    memory = cx.cpu4_update(memory, tb1, tn1, tn2) #ok
    cpu4 = cx.cpu4_output(memory) #ok
