...
def plot_route(h, v, T_outbound, T_inbound, plot_speed=False,
    ...
    if plot_speed:
        speed = np.clip(np.linalg.norm(np.vstack([np.diff(x), np.diff(y)]),
                        axis=0), 0, 1)
        n_min = np.argmin(speed[:T])
        n_max = np.argmax(speed[:T])
...
def plot_traces(log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'], n=0,
    ...
    data = {'TN1': log.tn1[:,:n], 'TN2': log.tn2[:,:n], 'TL2': log.tl2[:,:n], 'CL1': log.cl1[:,:n],
            'TB1': log.tb1[:,:n], 'CPU4': log.cpu4[:,:n], 'CPU1': log.cpu1[:,:n],
            'motor': log.motor[:n]}
    ...
    for i, cell_type in enumerate(include):
        ...
        if cell_type in ['TN1', 'TN2']:
            ...
            if cell_type == 'CPU1':
                ...
                fig.savefig('dummy.png')  # This is needed to force draw plot
